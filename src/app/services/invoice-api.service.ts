...
export class InvoiceAPIService {
  ...
  constructor (private readonly httpClient: HttpClient, private readonly ims: InvoiceModelServiceService) {
  }

  private getAuthKey (authKeyName: string): string {
    switch (authKeyName) {
      case 'faktura': return `${environment.authKeyFaktura}`
      case 'abonent': return `${environment.authKeyAbonent}`
      case 'rachunek': return `${environment.authKeyRachunek}`
      case 'wydatek': return `${environment.suthKeyWydatek}`
